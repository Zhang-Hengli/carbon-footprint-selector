...
def search_and_recommend(cas_number, product_name, process, remark):
    ...
    # 大类兜底触发条件：过滤掉1.25以上的条目后，剩下的数据少于20条
    CATEGORY_THRESHOLD = 1.25
    filtered_count = sum(1 for d in all_distances if d <= CATEGORY_THRESHOLD)
    need_category_fallback = filtered_count < 20
    
    if len(all_distances) > 0:
        avg_distance = sum(all_distances) / len(all_distances)
        max_distance = max(all_distances)
        logger.info(f"   检索质量指标: 平均距离={avg_distance:.3f}, 最大距离={max_distance:.3f}")
        logger.info(f"   阈值{CATEGORY_THRESHOLD}内候选: {filtered_count} 条")
    ...
    # ========== P0优化：过滤低质候选 + LLM打分 ==========
    DISTANCE_THRESHOLD = 1.25  # 放宽阈值，让更多候选通过
